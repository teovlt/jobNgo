...
interface LocationSelectorProps {
  ...
  state?: string; // Added state prop to set initial state value
  country?: string; // Added country prop to set initial country value
}
...
const LocationSelector = ({ disabled, onCountryChange, onStateChange, state, country }: LocationSelectorProps) => {
  ...
  // Find initial selected country and state safely
  const initialCountry = country ? countriesData.find((c) => c.name === country) || null : null;
  const initialAvailableStates = initialCountry ? statesData.filter((s) => s.country_id === initialCountry.id) : [];
  const initialState = state ? initialAvailableStates.find((s) => s.name === state) || null : null;

  const [selectedCountry, setSelectedCountry] = useState<CountryProps | null>(initialCountry);
  const [selectedState, setSelectedState] = useState<StateProps | null>(initialState);
